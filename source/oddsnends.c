...
void read_ids(char *idsfile,  char **ids1, char **ids2, char **ids3, int length, int *indexer, int head)
{
//currently assume ids in first two columns
int i, count, found, want;
...
found=0;
for(i=0;i<count;i++)	//read first two elements then skip to end of line
{
want=i;
if(indexer!=NULL){want=indexer[found];}
...
void print_empty()
{
printf("Error, you can only specify one argument from \"--cut-weights\", \"--calc-weights\", \"--join-weights\", \"--calc-weights-all\", \"--adjust-weights\", \"--thin\", \"--thin-tops\", \"--find-tags\", \"--remove-tags\", \"--cut-kins\", \"--calc-kins\", \"--join-kins\", \"--calc-kins-direct\", \"--add-grm\", \"--sub-grm\", \"--convert-gz\", \"--convert-raw\", \"--filter\", \"--calc-sim-grm\", \"--cut-genes\", \"--calc-genes-kins\", \"--calc-genes-reml\", \"--join-genes-reml\", \"--reml\", \"--calc-blups\", \"--he\", \"--pcgc\", \"--calc-traces\", \"--join-traces\", \"--fast-he\", \"--fast-pcgc\", \"--pca\", \"--calc-pca-loads\", \"--decompose\", \"--adjust-grm\", \"--truncate-grm\", \"--pca-grm\", \"--square-grm\", \"--gxemm-iid\", \"--gxemm-free\", \"--linear\", \"--logistic\", \"--solve-null\", \"--elastic-net\", \"--cut-net\", \"--calc-net\", \"--join-net\", \"--calc-net-all\", \"--calc-tagging\", \"--join-tagging\", \"--merge-tagging\", \"--reduce-tagging\", \"--sum-hers\", \"--sum-cors\", \"--calc-exps\", \"--calc-posts\", \"--calc-stats\", \"--calc-scores\", \"--make-phenos\", \"--make-snps\", \"--cut-folds\", \"--jackknife\", \"--make-bed\", \"--make-sp\", \"--make-sped\", \"--make-speed\", \"--make-gen\", \"--condense-bed\", \"--condense-sp\", \"--condense-sped\", \"--condense-speed\", \"--calc-sim-data\"\n\n");
}
...
if(size>1)
{printf("Warning, to store the region predictors will require approximately %.1f Gb\n\n", size);}
}
...
if(size>1)
{
printf("Warning, to store kinships will require approximately %.1f Gb", size);
...
if(size>1)
{printf("Warning, to store the eigen-decomposition requires approximately %.1f Gb; sorry, this can not be reduced\n\n", size);}
}
...
if(size>1)
{printf("Warning, to perform the decomposition will require approximately %.1f Gb; sorry, this can not be reduced\n\n", size);}
}
...
if(size>1)
{printf("Warning, to process the data will require approximately %.1f Gb; sorry this can not be reduced\n\n", size);}
}
...
size=(double)length1/1024*length2/1024*8/1024;
if(size>1)
{printf("Warning, to process the data will require approximately %.1f Gb; to reduce use \"--bit-size\" (currently %d)\n\n", size, length1);}
}
...
size=(double)length1/1024*length2/1024*8/1024;
if(size>1)
{printf("Warning, to process the data will require approximately %.1f Gb; to reduce use \"--bit-size\"\n\n", size);}
}
...
size=(double)length1/1024*length2/1024*8/1024;
if(size>1)
{printf("Warning, to perform the analysis will require approximately %.1f Gb; to reduce use \"--bit-size\" (currently %d)\n\n", size, length1);}
}
...
size=(double)length1/1024*length2/1024*8/1024;
if(size>1)
{printf("Warning, to store the heritability model will require approximately %.1f Gb\n\n", size);}
}
