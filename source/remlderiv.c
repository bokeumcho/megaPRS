...
//get KPY and trace PK for noise
...
if(shortcut==2)	//trace P = E(t(PR) * R)
{
sum=0;
for(g=0;g<num_vects;g++)
{
for(i=0;i<ns;i++){sum+=PR[i+g*ns]*R_single[i+g*ns];}
}
...
alpha_single=1.0;beta_single=0.0;
ssymv_("U", &ns, &alpha_single, kins_single, &ns, PY_single, &one, &beta_single, KPY_single, &one);
}
