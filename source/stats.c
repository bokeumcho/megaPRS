...
current=read_data_fly(datafile, dtype, data, ps, num_samples_use, keepsamps, bitstart, bitend, keeppreds_use, datainputgz, current, num_samples, num_preds, genskip, genheaders, genprobs, missingvalue, -9999, -9999, nonsnp, maxthreads);
...
else{fprintf(output2, "\t%.6f", (float)indcount/subindex[s][0]);}
if(genprobs>1){fprintf(output2, "\t%.4f", value);}
else{fprintf(output2, "\t-1");}
}	//end of s loop
