...
//write placeholders for counts, means, scalings and variances (these arrays were initialized in ldak.c)
...
//save noise
sprintf(filename5,"%s.cors.noise",outfile);
if((output5=fopen(filename5,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename2);exit(1);}
for(j=0;j<data_length;j++){fprintf(output5, "%.10f\n", datarands[j]);}
fclose(output5);
