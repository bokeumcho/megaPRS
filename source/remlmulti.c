...
void multi_reml(int ns, int num_covars, int num_envs, int num_tops, int num_kins, int num_regs, double *Y, double *Z, double **mkins, float **mkins_single, double *kintraces, double *kinsums, double *X, int Xtotal, int *Xstarts, int *Xends, double *Xsums, double prev, int np, char *hersfile, int hestart, int shortcut, double *U, double *E, int num_vects, int ldlt, int discenv, int liab, char *oversfile, double **ssums, int constrain, double tol, int maxiter, int memsave, int maxthreads, char **kinstems, char **ids3, char *outfile, char **ids1, char **ids2, int num_preds, char **allpreds, char *allal1, char *allal2, int *rkeeppreds, int **regindex, double *rcentres, double *rmults, double *rweights, int *tkeeppreds, double *tcentres, double *vstarts)
{
size_t scount, stotal;
int i, j, j2, k, k2, k3, r, g, count, token, flag, one=1, info, *ipiv, lwork;
...
float *R_single, **KR_single, *V_single, *RHS_single, *PY_single, *KPY_single, *RHS2_single, *kins_single;
...
V_single=malloc(sizeof(float)*ns*ns);
...
if(memsave==1){kins_single=malloc(sizeof(float)*ns*ns);}
...
{
read_kins(kinstems[k], NULL, kins_single, 1.0, ns, ids3, 4, maxthreads);

alpha_single=1.0;beta_single=0.0;
ssymm_("L", "U", &ns, &num_vects, &alpha_single, kins_single, &ns, R_single, &ns, &beta_single, KR_single[k], &ns);
}
}
}
...
if(shortcut==0||shortcut==1){he_starts(hers, ns, num_covars, num_envs, num_tops, num_kins, num_regs, Y, Z, mkins, NULL, kintraces, kinsums, X, Xtotal, Xstarts, Xends, Xsums, tol, memsave, kinstems, ids3);}
else{he_starts(hers, ns, num_covars, num_envs, num_tops, num_kins, num_regs, Y, Z, NULL, mkins_single, kintraces, kinsums, X, Xtotal, Xstarts, Xends, Xsums, tol, memsave, kinstems, ids3);}

//make sure no genetic heritabilities are not very small or negative (noise checked in next step)
for(k=1;k<total;k++)
{
...
if(count==maxiter)
{
if(shortcut==0||shortcut==1){printf("\nWarning, REML failed to converge within %d iterations; consider using \"--max-iter\" and/or \"--tolerance\" to change the iteration limit and tolerance", maxiter);}
if(shortcut==2){printf("\nWarning, REML failed to converge within %d iterations; consider using \"--max-iter\" and/or \"--tolerance\" to change the iteration limit and tolerance, or \"--repetitions\" to increase the number of random samplings", maxiter);}
if(constrain==0)
...
fprintf(output, "Component Heritability SD Size Mega_Intensity SD\n");
if(flag==0)	//might have null model
{
...
fprintf(output, "Component Heritability SD\n");
for(k=0;k<num_kins;k++)
{
sum=0;sum2=0;
...
fprintf(output, "Component Share SD Expected Enrichment SD\n");
for(k=0;k<num_kins;k++)
{
sum=0;sum2=0;
for(k2=0;k2<num_kins;k2++)
...
else	//load individuals into wandids, then read from file
{
for(i=0;i<count;i++){wantids[i]=ids3[Sindexer[i]];}
Sscales[1+k]=read_kin_trace(kinstems[k], count, wantids, 1);
}
}
...
if((output=fopen(filename,"w"))==NULL)
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename);exit(1);}
fprintf(output, "Component Num_Samples Heritability SD\n");
...
free(V_single);free(RHS_single);free(PY_single);free(KPY_single);free(RHS2_single);if(memsave==1){free(kins_single);}
