...
{printf("Will first thin so that there are no pairs of predictors within %d with correlation squared greater than %.4f", window_length, wprune);}
...
{printf("When calculating how well each predictor is tagged, will consider correlations with predictors within %d", window_length);}
...
if(mode==103)
{
if(num_sections==1){printf("Joining weights across 1 section\n\n");}
...
{printf("so that no pair less than %d apart remain with correlation squared greater than %.4f\n\n", window_length, wprune);}
...
if(mode==107)
{
printf("Will identify predictors with p-values less than %.2e, then thin ", cutoff);
if(window_kb!=-9999)
{
if(window_cm!=-9999)
...
{printf("so that no pair less than %d apart remain with correlation squared greater than %.4f ", window_length, wprune);}
printf("(predictors with higher p-values will be excluded first)\n\n");
}
...
{
if(num_parts==1){printf("Cutting predictors into 1 partition according to %s1\n\n", partpref);}
if(num_parts==2){printf("Cutting predictors into 2 partitions according to %s1 and %s2\n\n", partpref, partpref);}
...
if(mode==113)
{
if(num_parts==1){printf("Joining kinships across 1 partition\n\n");}
...
{
if(num_kins==1){printf("with 1 kinship matrix ");}
else{printf("with %d kinship matrices ", num_kins);}
if(num_regs==1){printf("and 1 region\n\n");}
...
if(strcmp(sumsfile,"blank")!=0&&shrink==1&&strip==0){printf("Consider using \"--shrink\" or \"--strip\" in an attempt to reduce the noise in the regional correlation matrix\n\n");}
}

if(strcmp(sumsfile,"blank")==0)
{
if(strcmp(covarfile,"blank")==0){printf("Consider using \"--covar\" to provide covariates\n\n");}
if(strcmp(topfile,"blank")==0){printf("Consider using \"--top-preds\" to include (strongly-associated) predictors; these will be treated as fixed effects, except that the variance they explain will count towards total heritability\n\n");}
}
...
if(permute==0){printf("If you would like to perform a permutation analysis, add \"--permute YES\"\n\n");}
}
...
if(mode==127)
{
printf("Performing fast Haseman Elston Regression ");
if(num_parts>0)
{
if(parttype==0)
{
if(num_parts==1){printf(" using a base category and 1 annotation\n");}
else{printf(" using a base category and %d annotations\n", num_parts);}
}
else
{
if(num_parts==1){printf(" using 1 partition\n");}
...
if(mode==128)
{
printf("Performing fast PCGC Regression ");
if(num_parts>0)
{
if(parttype==0)
{
...
if(mode==131)
{
if(num_kins==0){printf("Performing (standard) linear regression (to instead perform mixed model linear regression, use \"--grm\")\n\n");}
else
{
...
if(exact==1){printf("Variance components will be re-estimated for each predictor; if you add \"--exact NO\", they will be fixed under the null (which will reduce the runtime a bit)\n\n");}
}
...
if(num_fixed>1){printf("A predictor will be ignored if it has correlation squared greater than %.2f with any covariate (to change this threshold use \"--min-cor\")\n\n", mincor);}
...
if(num_fixed>1){printf("A predictor will be ignored if it has correlation squared greater than %.2f with any fixed effect (to change this threshold use \"--min-cor\")\n\n", mincor);}
...
if(gene_perms>0)
{
if(gene_perms==1){printf("Will analyse the observed phenotypes and 1 permutation ");}
...
if(shrink>1)
{printf("Will scale diagonal values of the regional correlation matrix by %.4f (to change this value use \"--shrink\")\n\n", shrink);}
if(strcmp(sumsfile,"blank")!=0&&shrink==1){printf("Consider using \"--shrink\" in an attempt to reduce the noise in each gene/chunk correlation matrix\n\n");}
...
if(mode==139)
{
if(num_parts==1){printf("Joining results from gene/chunk-based REML across 1 partition\n\n");}
...
if(window_cm!=-9999)
{printf("Will consider correlations with predictors within %.4fcM", window_cm);}
else
{printf("Will consider correlations with predictors within %.2fkb", window_kb);}
if(mincor>0){printf("; correlation squared values less than %.4f are considered noise and set to zero", mincor);}
printf("\n\n");
...
if(mode==144)
{
printf("Reducing a tagging file; ");
if(parttype==0)
{
...
if(parttype==0)
{
if(num_parts==2){printf("Estimating heritabilities from summary statistics with the base category and 1 annotation\n");}
...
printf(" (change this using \"--genomic-control\" and \"--intercept\")\n\n");
...
if(parttype==0)
{
if(num_parts==2){printf("Estimating genetic correlations from summary statistics with the base category and 1 annotation\n");}
...
if(mode==151)
{
printf("Constructing a ridge regression prediction model\n\n");

if(strcmp(covarfile,"blank")==0){printf("Consider using \"--covar\" to provide covariates\n\n");}
if(strcmp(topfile,"blank")==0){printf("Consider using \"--top-preds\" to include (strongly-associated) predictors; their effects will be estimated via least-squares regression (i.e., not shrunk like regular predictors)\n\n");}

if(restart==0){printf("Note that if this job fails to complete (due to time), it can be resumed by resubmitting the same command but adding \"--restart YES\"\n\n");}
}

if(mode==152)
{
printf("Constructing a Bolt-LMM prediction model\n\n");

printf("Will consider six values for p (the proportion of large-effect predictors) and three values for f2 (the proportion of heritability the small-effect predictors explain)");
if(cvprop!=-9999||strcmp(bvsfile,"blank")!=0){printf(", using cross-validation to decide which is best");}
printf("\n\n");
...
if(mode==153)
{
printf("Constructing a BayesR prediction model\n\n");

if(pointmass==1){printf("To replace the point mass in the prior with a Gaussian distribution (with variance 1/1000 times that of the largest variance), use \"--point-mass NO\")\n\n");}

if(fullspace==0)
{
printf("Will consider 35 combinations for (pi1, pi2, pi3), the fractions corresponding to the three largest variance terms");
if(cvprop!=-9999||strcmp(bvsfile,"blank")!=0){printf(", using cross-validation to decide which is best");}
printf("; to instead consider 124 combinations, use \"--full-space YES\"\n\n");
}
else
{
printf("Will consider 124 combinations for (pi1, pi2, pi3), the fractions corresponding to the three largest variance terms");
if(cvprop!=-9999||strcmp(bvsfile,"blank")!=0){printf(", using cross-validation to decide which is best\n\n");}
}
...
if(mode==155)
{
if(window_cm!=-9999){printf("Calculating correlations between pairs of predictors within %.4fcM, ", window_cm);}
else{printf("Calculating correlations between pairs of predictors within %.2fkb, ", window_kb);}
if(mincor==-9999){printf("saving those that are significant (P<0.01); to instead specify a correlation squared threshold use \"-min-cor\"\n\n");}
else{printf("saving when the correlation squared is greater than %.4f\n\n", mincor);}
}
...
if(ptype==7){printf("will consider lasso, ridge regression, Bolt-LMM and BayesR models\n\n");}

if(ptype==2&&multiher==1){printf("Will consider 11 lasso models (corresponding to scaling the per-predictor heritabilities by between 0.5 and 1.5); to consider only one model, use \"--multi-hers NO\"\n\n");}
if(ptype==3&&multiher==1){printf("Will consider 11 ridge regression models (corresponding to scaling the per-predictor heritabilities by between 0.5 and 1.5); to consider only one model, use \"--multi-hers NO\"\n\n");}

if(ptype==4&&basicbolt==0&&ldpred==0){printf("Will consider 12 values for p (the proportion of large-effect predictors) and 11 values for f2 (the proportion of heritability the small-effect predictors explain); to consider only six values for p and three for f2 (the same as when using individual-level data), use \"--basic-bolt YES\", while to consider 22 values for p and set f2=0, use \"--LDpred YES\"\n\n");}
...
if(strcmp(sums2file,"blank")!=0&&(ptype!=2||ptype!=3||multiher==1))
...
if(mode==160)
{
if(num_scores==1){printf("Measuring the accuracy of 1 prediction model\n\n");}
...
if(mode==171)
{
printf("Calculating predictor and individual statistics\n\n");
}

if(mode==172)
{
if(num_scores==1){printf("Calculating scores for 1 profile\n\n");}
...
if(mode==173)
{
if(num_phenos==1){printf("Making 1 phenotype, with heritability %.4f and ", her);}
...
if(bivar==-9999){printf("To generate correlated pairs of phenotypes, use \"--bivar\"\n\n");}
else{printf("Pairs of phenotypes will have correlation %.4f\n\n", bivar);}
}

if(mode==174)
{
printf("Making genotypes for %d individuals and %d SNPs, each with %.4f <= MAF <= %.4f (change these limits using \"--maf-low\" and \"--maf-high\")\n\n", num_inds, num_snps, maf1, maf2);
}
...
{
if(speedlong==0)
{printf("Each predictor will be stored as 1 byte (accurate to 1/254); this is normally sufficient, but to increase this to 2 bytes (accurate to 1/65534) use \"--speed-long YES\"\n\n");}
else
{printf("Each predictor will be stored as 2 bytes (accurate to 1/65534); note that you can not use this format with \"--ridge\", \"--bolt\" or \"--bayesr\"\n\n");}
}
}
