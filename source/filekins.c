...
{
threadstart=pow((float)thread/maxthreads,.5)*ns;
threadend=pow((float)(thread+1)/maxthreads,.5)*ns;
...
{printf("Error reading %s; Row %jd starts %d %d (not %d %d)\nIf this is a partial kinship matrix (only contains values for a subset of individuals), please add \"--partial YES\"\n\n", filename, scount+1, readi, readi2, i+1, i2+1);exit(1);}
...
//type=0 - quiet, type=1 - normal, type=2 - null for association testing, type=3 - trun/pca/square/gxemm kinships, type=4 - convert
{
int i, i2, j, count;
float *datatemp;
...
system(cmd);
if(type==1||type==4){printf(", with gzipped version saved in %s.gz", filename5);}
}
