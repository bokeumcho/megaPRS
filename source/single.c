...
//fill Y and start of Z, and if using, get factor
...
//solve null model - get thetas for covariates, maybe adjust response, and when using a kinship compute starting variances
if(mode==131)
{
if(num_kins==0)
...
if(mode==131&&num_kins==0)	//get variance of adjusted phenotype
{
...
//read data for chunk, and just centre (and replace NAs with zero)
...
if(mode==131&&num_kins==0)	//get YTdata (using adjusted phenotypes), maybe regress covariates out of data, get XTCXs
{
alpha=1.0;beta=0.0;
dgemv_("T", &num_samples_use, &bitlength, &alpha, data, &num_samples_use, Yadj, &one, &beta, YTdata, &one);

if(num_fixed>1||strcmp(sampwfile,"blank")!=0){reg_covar_matrix(data, Z, num_samples_use, bitlength, num_fixed);}

#pragma omp parallel for private(j, i) schedule(static)
for(j=bitstart;j<bitend;j++)
{
XTCX[j-bitstart]=0;for(i=0;i<num_samples_use;i++){XTCX[j-bitstart]+=pow(data[i+(j-bitstart)*num_samples_use],2);}
}
}
...
if(num_kins==0)	//basic
{
//compute YTCX/XTCX and YCT(YCT-XCvalue)/XTX(n-nf-1)
value=YTdata[j-bitstart]/XTCX[j-bitstart];
value2=(varphen*num_samples_use/XTCX[j-bitstart]-pow(value,2))/(num_samples_use-num_fixed-1);
...
{
//add test predictor to end of Z and test
for(i=0;i<num_samples_use;i++){Z[i+num_fixed*num_samples_use]=data[(size_t)(j-bitstart)*num_samples_use+i];}
reg_single_log(Y, Z, num_samples_use, num_fixed+1, stats, tol, maxiter);
}
}	//end of not a top
}	//end of testing
...
fprintf(output3, "%s %c %c %d %.4f %d\n", preds[j], al1[j], al2[j], (stats[0]>=0)-(stats[0]<0), pow(stats[2],2), num_samples_use);
...
if(mode==131&&num_kins==0){free(YTdata);free(XTCX);}
