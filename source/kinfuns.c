...
double *centres, *mults, *weights, *kcentres, *kmults, *kweights;
...
{printf("Error, the kinship matrices %s and %s were constructed from different data files (%s and %s); re-examine your commands, or override this warning by manually editing the \".grm.adjust\" files\n\n", kinstems[0], kinstems[k], datafile, readstring2);exit(1);}
flag=1;
}
}
...
read_details(filename, kpreds, kindex, kcentres, kmults, kweights, kal1, kal2, count);
...
if(type==0||type==1||type==3){weights[j2]+=kweights[j]*value;}
else
{
weights[j2]-=kweights[j]*value;
if(weights[j2]<0){printf("Error reading %s, predictor %s has been subtracted \"too much\"\n\n", filename2, preds[j2]);exit(1);}
}
}	//end of already used
}	//end of j loop

for(j=0;j<count;j++){free(kpreds[j]);}free(kpreds);
free(kindex);free(kcentres);free(kmults);free(kweights);free(kal1);free(kal2);
}	//end of k loop
...
//now write
if(flag==0)
{write_kins(outfile, NULL, kins_single, ns, ids1, ids2, 1, preds, keeppreds_use, centres, mults, weights, al1, al2, length, datafile, power, kingz, kinraw, 1+(type==3));}
else
{write_kins(outfile, NULL, kins_single, ns, ids1, ids2, 1, preds, keeppreds_use, centres, mults, weights, al1, al2, length, NULL, power, kingz, kinraw, 1+(type==3));}

////////

for(j=0;j<length;j++){if(usedpreds[j]==1){free(preds[j]);}}
free(preds);free(usedpreds);
free(centres);free(mults);free(weights);free(al1);free(al2);
