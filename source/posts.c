...
for(j=count2;j<count2;j++){free(preds[j]);}
count=count2;
}
...
//for each SNP compute posterior mean and variance (assume Var(X)=Var(Y)=1)
...
fprintf(output,"Predictor A1 A2 Posterior_Mean Posterior_Variance Log10_Bayes_Factor Test_Stat P\n");
for(j=0;j<count;j++)
{
fprintf(output, "%s %c %c ", preds[j], al1[j], al2[j]);
fprintf(output, "%.4e %.4e %.4f %.4f %.4e\n", pmeans[j], pvars[j], bfs[j]/log(10), pow(pmeans[j],2)/pvars[j], erfc(fabs(pmeans[j]*pow(pvars[j],-.5))*M_SQRT1_2));
}
fclose(output);
...
free(pmeans);free(pvars);free(bfs);
