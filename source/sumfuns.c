...
void solve_sums(double *stats, double *likes, double *cohers, double *influs, int num_parts, int gcon, int cept, int num_blocks, int length, int ncv, int *cvindex, double *cvexps, double *stags, double **svars, double **ssums, double *snss, double *schis, int parttype, double tol, int maxiter, int chisol, int sflag, char *filename)
//sflag=0 - normal, sflag=1 - first pass, sflag=2 - second pass
//sflag=3 - just get expectations and likelihood, sflag=4 - LDSC, sflag=5 - divide+updating
{
...
//assuming statj = c * (1 + nja + njvj b), where c is gif, a is intercept/n, b is h2SNP/ssums
...
count=0;
cflag=1;
rflag=1;	//0 if using multiNR (or least-squares), 1 if about to do singleNR, 2 if just done singleNR
while(1)
{
...
if(ncv==0&&(sflag==0||sflag==2||sflag==4||sflag==5))	//get SDs
{
...
for(q2=0;q2<num_parts;q2++){jacks[total+1+q+mark]+=thetas[q2]/gc*ssums[q][q2];}
}
}	//end of p loop
printf("\n");
...
void solve_cors(double *stats, int num_parts, int gcon, int cept, int num_blocks, int length, double *stags, double **svars, double **ssums, double *snss, double *schis, double *srhos, double *snss2, double *schis2, double *srhos2, int parttype, double tol, int maxiter, char *filename)
{
...
sW[j]=stags[j]*pow(exps[j],2);
if(sW[j]<=0){sW[j]=1e-6;}
for(q=0;q<num_parts;q++){sX[j+q*length]=snss[j]/scale*svars[q][j]*pow(sW[j],-.5);}
if(gcon==1){sX[j+num_parts*length]=pow(sW[j],-.5);}
if(cept==1){sX[j+(num_parts+gcon)*length]=snss[j]/scale*pow(sW[j],-.5);}
...
//put heritabilities, their sum, gc and cept into first column of stats (sum goes right at end)
gc=1;if(gcon==1){gc=thetas[num_parts]+1;}
sumhers=0;for(q=0;q<num_parts;q++){sumhers+=thetas[q]/gc*ssums[q][q]/scale;}
for(q=0;q<num_parts;q++){stats[q]=thetas[q]/gc*ssums[q][q]/scale;}
if(gcon==1){stats[num_parts]=gc;}
...
if(sW[j]<=0){sW[j]=1e-6;}
for(q=0;q<num_parts;q++){sX[j+q*length]=snss2[j]/scale2*svars[q][j]*pow(sW[j],-.5);}
if(gcon==1){sX[j+num_parts*length]=pow(sW[j],-.5);}
if(cept==1){sX[j+(num_parts+gcon)*length]=snss2[j]/scale2*pow(sW[j],-.5);}
...
//put heritabilities, their sum, gc and cept into first column of stats (sum goes rightat end)
gc2=1;if(gcon==1){gc2=thetas[num_parts]+1;}
sumhers2=0;for(q=0;q<num_parts;q++){sumhers2+=thetas[q]/gc2*ssums[q][q]/scale2;}
for(q=0;q<num_parts;q++){stats[total2+q]=thetas[q]/gc2*ssums[q][q]/scale2;}
if(gcon==1){stats[total2+num_parts]=gc2;}
if(cept==1){stats[total2+num_parts+gcon]=thetas[num_parts+gcon]/gc2;}
...
if(cept==1){jacks[total2+num_parts+gcon+mark]=thetas[num_parts+gcon]/gc2;}
...
//put coheritabilities, their sum and the weird term into first column of stats (sum goes right at end)
gc=1;if(gcon==1){gc=stats[num_parts];}
gc2=1;if(gcon==1){gc2=stats[total2+num_parts];}
gc3=pow(gc*gc2,.5);
