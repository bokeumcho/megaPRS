...
if(use_data==1||use_data==2||use_data==3||use_data==4||num_kins>0||mode==121||mode==123||mode==124||mode==126||mode==177)	//will be using sample ids - if change this, remember to update free in ldak.c
{
...
if((mode==121||mode==126||mode==133)&&hestart==-9999)	//set he start
{
//use starts from he regression, unless not possible due to memsave
hestart=1;
if(num_kins>1&&memsave==1)
{
for(k=1;k<num_kins;k++)
{
sprintf(filename2,"%s.grm.id",kinstems[k]);
count2=countrows(filename2);
if(count2!=count){hestart=0;break;}

kinids2=malloc(sizeof(char*)*count2);
read_ids(filename2, NULL, NULL, kinids2, count2, NULL, 0);
for(i=0;i<count2;i++)
{
if(strcmp(kinids[i],kinids2[i])!=0){hestart=0;break;}
}
for(i=0;i<count2;i++){free(kinids2[i]);}free(kinids2);
}
}
...
else	//either get from data file or response
{
...
if(mode==141&&num_samples_use>10000)
{printf("Warning, there is normally no need to use this many samples when calculating taggings; if the analysis takes a long time, consider using \"--keep\" to specify, say, 5000 samples, picked at random\n\n");}
...
read_bimfile(bimfile, allchr, allpreds, allcm, allbp, allal1, allal2, num_preds, NULL, 1, window_cm, 0);
}
