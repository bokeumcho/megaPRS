...
{
//only need nss2, rhos2 and rhos3 
...
{
//will also read nss3, chis2 and chis3
...
if(skipcv==0)	//set ldpreds, which indicates which predictors to use for testing
{
...
//read maxnums from cors and use to set scumsums
...
if(skipcv==0){predcors=malloc(sizeof(double)*num_try);}
...
#pragma omp parallel for private(p,j,j2,value,value2) schedule(dynamic, 1)
for(p=0;p<num_try;p++)
{
...
{
//ensure best model is set - if using cv, will already be set, else will have num_try=1
if(skipcv==0){best=0;}
