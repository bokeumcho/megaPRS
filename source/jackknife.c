...
if(count==2){printf("Reading predicted and observed values for %d individuals from %s\n", count2, jackfile);}
else{printf("Reading predicted and observed values for %d individuals, as well as weightings, from %s\n", count2, jackfile);}
...
if(auc==1)	//check observed values are zero or one (but not all same) and sort
{
total=0;
for(j=0;j<count2;j++)
{
...
else	//using profile file
{
//read observed values into second column of sX, and predictions into guesses
guesses=malloc(sizeof(double*)*count);
for(k=0;k<count;k++){guesses[k]=malloc(sizeof(double)*count2);}
...
for(j=0;j<count2;j++)
{
if(fscanf(input, "%s %s %s %s", readstring, readstring, readstring2, readstring)!=4)
{printf("Error reading first four values of Row %d of %s\n\n", j+1, proffile);exit(1);}
if(strcmp(readstring2,"NA")==0){printf("Error reading Row %d of %s; the phenotype is missing\n\n", j+1, proffile);exit(1);}
sX[count2+j]=atof(readstring2); 
for(k=0;k<count;k++)
{
if(fscanf(input, "%lf %s ", guesses[k]+j, readstring)!=2)
...
if(auc==1)	//check observed values are zero or one (but not all same)
{
total=0;
for(j=0;j<count2;j++)
{
...
if(total==count2){printf("Error reading %s; all phenotypes are one\n\n", proffile);exit(1);}
}

for(k=0;k<count;k++)
{
//load predicted values for Profile k into first column of sX
for(j=0;j<count2;j++){sX[j]=guesses[k][j];}
...
free(sX);free(sXTX);free(sXTX2);free(sW);
free(stats);
