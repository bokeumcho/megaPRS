...
//here is code when used to switch to eigendecomposition (saved space by using P as workspace)
//must first remake V
//detV=eigen_invert(V, ns, V2, -1, P, 1);
}
}	//end of shortcut=0
...
for(i=0;i<ns;i++){V_single[(size_t)i*ns+i]=vars[0];}
...
free(work_single);
if(info!=0)
{printf("Error, LDLT decompostion failed, please tell Doug (info %d, length %d)\n\n", info, ns);exit(1);}
}
...
//ultimately want PY, where P = invV - invVZ invZTVZ ZTinvV or U (invD-DUTXF-BUTZH) UT, H = invZTVZ t(BUTZ)
