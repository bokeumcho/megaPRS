...
count=0;for(j=0;j<data_length;j++){count+=(mults[j]!=-9999&&vindex[j]==1);}
count2=0;for(j=0;j<data_length;j++){count2+=(mults[j]!=-9999&&windex[j]==1);}
...
//allocate variables
data_warn3(bitsize,num_samples_use);
...
stand_data(data, centres+bitstart, mults+bitstart, sqdevs+bitstart, num_samples_use, bitlength, missingvalue, -1, 0, 0, NULL, 0, preds+bitstart);
}	//end of bit loop
printf("\n");
