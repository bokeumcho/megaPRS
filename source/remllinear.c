...
double **mkins=NULL, *X=NULL, *Xsums=NULL;
...
if(exact==0){break;}	//only required BUTZ and ZTVZ
...
if(stats==NULL)	//save thetas and heritabilities
{
for(j=0;j<num_fixed;j++){ts[j]=thetas[j];tsds[j]=thetasds[j];tpvas[j]=thetapvas[j];}
vstarts[0]=hers[0];vstarts[1]=hers[1];
}
else
{
//load up stats
