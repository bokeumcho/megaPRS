...
int read_bed_fly(char *bedfile, double *data, int num_samples_use, int *keepsamps, int start, int end, int *keeppreds, int num_samples, int num_preds, double missingvalue)
{
int i, j, count, count2, current, rowlength;
unsigned char startchars[3], *rowchars, twobits;
...
float read_speed_size(FILE *input, char *speedfile)
{
float size;
...
size=read_speed_size(Minput[0], speedfile);
if(size==1)
{
Mrowchars=malloc(sizeof(unsigned char*)*maxthreads);
...
{printf("Error, %s does not contain SNP allele counts; you should use \"--SNP-data NO\"\n\n", speedfile);exit(1);}
...
size=read_speed_size(input, speedfile);
...
{
int i, k, found, wcount, offset, last, size, flag;
...
if(keeppreds[start+found]==last)	//then copy in previous row
{
for(i=0;i<num_samples_use;i++)
{
...
last=keeppreds[start+found];found++;current++;
