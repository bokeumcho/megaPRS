...
if(append_check(propfile,propfile2,workdir)!=0)
{printf("Error reading proportions file %s; check file exists and is not empty\n\n", propfile);exit(1);}
...
(void)append_check(corstem,corstem2,workdir);
...
(void)append_check(jackstem,jackstem2,workdir);
