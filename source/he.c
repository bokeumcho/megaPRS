...
int he_reg(int ns, int num_subs, int num_covars, int num_envs, int num_tops, int num_kins, int num_regs, int **subindex, double *Y, double *Z, float **mkins_single, double *kintraces, double *kinsums, double *X, int Xtotal, int *Xstarts, int *Xends, double *Xsums, double prev, int np, int discenv, char *oversfile, double **ssums, int num_blocks, double tol, int memsave, char **kinstems, char **ids3, char *outfile, int type)
...
double *thetas, *thetasds, *thetapvas, *Yadj, topher, covher;
...
{reg_covar_log(Y, Z, ns, num_covars+num_envs, num_tops, thetas, thetasds, thetapvas, Yadj, &covher, &topher, prev, 0.0001, 20);}
...
//solve for all
printf("Analysing %jd pairs of samples\n", scounta+scountb);
...
{printf("Error writing to %s; check you have permission to write and that there does not exist a folder with this name\n\n",filename3);exit(1);}
if(type==0){fprintf(output3, "Component Effect SD P\n");}
...
else{Sscales[k]=read_kin_trace(kinstems[k], count, wantids, 1);}
}
for(r=0;r<num_regs;r++)
{
sum=0;
