...
{
if(pow(cors[(size_t)(k-bitstart)*bitlength+(j-bitstart)],2)>=wprune)	//will lose i, keep i2
{
i=j;i2=k;
if(j+k%2==1){i=k;i2=j;}
...
//squeeze down (not usedpreds, while mults and centres will be recomputed)
count=0;
for(j=0;j<data_length;j++)
{
if(mults[j]!=-9999)
...
printf("Thinning complete: %d predictors kept (saved in %s), %d lost (%s), %d trivial (%s)\n", count, filename2, count2, filename3, count3, filename4);
if(mode==101)
