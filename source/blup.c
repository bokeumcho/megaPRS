...
double value, value2, **allcentres, **allfactors, *centres, *mults, *weights, *factors;
char **preds, *al1, *al2;

char readchar, readstring[500], *rs, *ra1, *ra2, *rc, *re;
...
rc=malloc(sizeof(char)*10000000);re=malloc(sizeof(char)*10000000);
...
read_details(filename, preds, indexer, centres, mults, weights, al1, al2, count);
...
free(indexer);free(centres);free(mults);free(weights);free(al1);free(al2);
}	//end of k loop
...
free(rs);free(ra1);free(ra2);free(rc);free(re);
return(count);
}	//end of read_effects
...
if(value>1){printf("Warning, this will require %.1f Gb\n\n", value);}
...
if(fscanf(input, "%s ", rc)!=1)
{printf("Error reading centre for Predictor %s in %s\n\n", preds[j], efffile);exit(1);}
if(strcmp(rc,"NA")==0){centres[j]=-9999;}	//NAs allowed
else
{printf("Error reading %s; centre for Predictor %s is unrecognisable (%s)\n\n", efffile, preds[j], rc);exit(1);}
}
...
for(r=0;r<num_scores;r++)
{
if(fscanf(input, "%[0-9eE.+-] ", re)==1){factors[r][j]=atof(re);}
else
{
if(fscanf(input, "%s ", re)!=1)
{printf("Error reading Effect %d for Predictor %s in %s\n\n", r+1, preds[j], efffile);exit(1);}
...
//set keeppreds_use and indexer2
count2=find_strings(allpreds, num_preds, preds, count, keeppreds_use, indexer2, NULL, efffile, predorder, NULL, 3);
if(count2==0){printf("Error, none of these are in the data\n\n");exit(1);}
...
for(j=0;j<count;j++){free(preds[j]);}free(preds);
free(al1);free(al2);free(centres);free(factors);free(indexer2);
...
if(fscanf(input, "%s%c", re, &readchar)!=2)
...
void write_scores(double **guesses, int **nums, int ns, int num_scores, char **ids1, char **ids2, char *outfile, double *resp, double missingvalue, int type)
//type=0, save only scores, type=1, save also counts
{
int i, r;
