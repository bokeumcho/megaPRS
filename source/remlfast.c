...
num_fixed=num_covars+num_tops+num_envs;
if(Xnss==NULL)
{
nfree=ns-num_fixed;
scale=1;
...
//get SDs of hers - Jij=delta/sum-vari/sum^2 (where sum across all elements)
sum=0;for(r=0;r<total;r++){sum+=vars[r];}
...
hers[0]=0;for(r=1;r<total;r++){hers[0]+=hers[r];}
sum=0;
for(r=1;r<total;r++)
{
...
//get SDs of shares - Jij=delta/sum-vari/sum^2 (where sum excludes noise term)
sum=0;for(r=1;r<total;r++){sum+=vars[r];}
...
if(num_covars>1){printf("Poportion of variance explained by the %d covariates: %.4f\n", num_covars, covher);}
...
if(Xnss==NULL){fprintf(output2, "Blupfile %s.indi.blp.liab\n", outfile);}
else{fprintf(output2, "Blupfile none\n");}
fprintf(output, "Regfile %s.reg.blup.liab\n", outfile);
if(Xnss==NULL){fprintf(output2, "Coeffsfile %s.coeff.liab\n", outfile);}
else{fprintf(output2, "Coeffsfile none\n");}
fprintf(output2, "Covar_Heritability %.4f\n", covher*factor);
if(Xnss==NULL){fprintf(output2, "Total_Samples %d\nWith_Phenotypes %d\n", ns, np);}
else{fprintf(output2, "Num_Genotyped_Samples %d\nAverage_Sample_Size %.2f\n", ns, nfree);}
