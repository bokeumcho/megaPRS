...
read_ids(famstems[k], Xids1[k], Xids2[k], Xids3[k], Xnall[k], NULL, famhead);
...
if(num_samples_use<num_samples){printf("Will be using %d of these\n", num_samples_use);}
printf("\n");
...
read_bimfile(bimstems[k], Xchr[k], Xpreds[k], Xcm[k], Xbp[k], Xal1[k], Xal2[k], XNall[k], NULL, 1, -9999, (binary==1&&usenames==1));
}
...
{Xcurrent[k]=read_data_fly(datastems[k], dtype, data2, ps2, Xnuse[k], Xks[k], bitstart2, bitend2, Xkp[k], Xdatainputgz[k], Xcurrent[k], Xnall[k], XNall[k], genskip, genheaders, genprobs, missingvalue, threshold, minprob, nonsnp, 1);}
else	//read just data
{Xcurrent[k]=read_data_fly(datastems[k], dtype, data2, NULL, Xnuse[k], Xks[k], bitstart2, bitend2, Xkp[k], Xdatainputgz[k], Xcurrent[k], Xnall[k], XNall[k], genskip, genheaders, genprobs, missingvalue, threshold, minprob, nonsnp, maxthreads);}
...
else{fprintf(output2, "\t%.6f", (float)indcount/num_samples_use);}
...
if(mode!=190)	//write bimfile, famfile and .qc
{
...
printf("Gzipping %s; if this fails to complete, run the command\n%s\n\n", filename3, cmd);
system(cmd);
}
}	//end of quickmerge=0
