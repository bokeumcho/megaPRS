...
inputs=malloc(sizeof(FILE*)*num_files);
Xcurrent=malloc(sizeof(int)*num_files);

for(k=0;k<num_files;k++)
{
if((inputs[k]=fopen(datastems[k],"rb"))==NULL)
{printf("Error opening %s\n\n",datastems[k]);exit(1);}

fseeko(inputs[k], 0, SEEK_END);
if(ftello(inputs[k])!=(off_t)sizeof(unsigned char)*rowlength*XNall[k]+sizeof(unsigned char)*3)
{printf("Error reading %s; should have size %jd (%d ind x %d predictors), but instead has size %jd\n\n", datastems[k], (off_t)sizeof(unsigned char)*rowlength*XNall[k]+sizeof(unsigned char)*3, num_samples_use, XNall[k], ftello(inputs[k]));exit(1);}

fseeko(inputs[k], 0, SEEK_SET);
if(fread(startchars, sizeof(unsigned char), 3, inputs[k])!=3)
...
if(j2!=Xcurrent[k])
{
if(fseeko(inputs[k], (off_t)sizeof(unsigned char)*rowlength*j2+sizeof(unsigned char)*3, SEEK_SET)!=0)
{printf("Error reading %s; unable to find Predictor %d\n\n", datastems[k], j2+1);exit(1);}
}
if(fread(rowchars, sizeof(unsigned char), rowlength, inputs[k])!=rowlength)
...
for(k=0;k<num_files;k++){fclose(inputs[k]);}
fclose(output2);
...
free(rowchars);
free(inputs);free(Xcurrent);

printf("Data for %d samples and %d predictors saved in %s, %s and %s\n\n", num_samples_use, count, filename2, filename3, filename4);
