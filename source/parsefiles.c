...
if(mode==101||mode==102||mode==103||mode==104||mode==105||mode==106||mode==107||mode==108||mode==109||mode==111||mode==112||mode==114||(mode==117&&extract==1)||mode==122||mode==127||mode==131||mode==132||mode==136||mode==137||mode==138||(mode==139&&cut1!=-9999)||mode==141||mode==145||mode==151||mode==152||mode==153||mode==155||mode==157||mode==158||mode==160||mode==162||mode==171||mode==172||mode==173||mode==186||mode==187||mode==188||mode==189||mode==191||mode==192||mode==193||mode==194)
{use_data=1;}
...
{
strcpy(datafile,"blank");sprintf(bimfile,"%s.cors.bim", corstem);strcpy(famfile,"blank");
}
...
{
if(mode==101||mode==102||mode==103||mode==104||mode==106||mode==107||mode==108||mode==109||mode==111||mode==117||mode==122||mode==131||mode==132||mode==136||mode==139||mode==155||mode==157||mode==158||mode==160||mode==162||mode==171||mode==172||mode==191||mode==192||mode==193||mode==194)	//not required, can set to one
{ignoreweights=1;}
else	//weights are required
{
if(mode==137||mode==138)
{printf("Error, you must use either \"--weights\" or \"--ignore-weights YES\"; for gene/chunk-based analyses, it is usually OK to ignore weightings\n\n");exit(1);}
if(mode==151||mode==152||mode==153||mode==159)
...
{
if(mode!=112&&mode!=114&&mode!=127&&mode!=137&&mode!=138&&mode!=141&&mode!=145&&mode!=151&&mode!=152&&mode!=153&&mode!=159&&mode!=172&&mode!=173&&mode!=186&&mode!=187&&mode!=188&&mode!=189&&num_regs==0)
...
if(mode==121||mode==123||mode==124||mode==126||mode==163)
...
{
if(mode==121||mode==122||mode==123||mode==124||mode==126||mode==127||mode==128||mode==131||mode==132||mode==133||mode==138||mode==151||mode==152||mode==153||mode==172||mode==194)	//will be using covar
{num_covars=1;}
else
{num_covars=0;}
}
...
if(count!=3)
{printf("Error, %s should have three columns (not %d), containing the two sample IDs then the weight\n\n", sampwfile, count);exit(1);}
}
...
if(strcmp(corstem,"blank")!=0)	//check files
{
//first read root - only need num_preds_used and num_pairs
sprintf(filename,"%s.cors.root", corstem);
if(just_check(filename)!=0)
...
sprintf(filename,"%s.cors.bin", corstem);
if(just_check(filename)!=0)
{printf("Error reading %s; this file would have been created using \"--calc-cors\"\n\n", filename);exit(1);}
if((input=fopen(filename,"rb"))==NULL)
{printf("Error opening %s\n\n",filename);exit(1);}
...
//check also bim file exists and correct size
sprintf(filename,"%s.cors.bim", corstem);
if(just_check(filename)!=0)
{printf("Error reading %s; this file would have been created using \"--calc-cors\"\n\n", filename);exit(1);}
...
//and lastly check noise file exists
sprintf(filename,"%s.cors.noise", corstem);
if(just_check(filename)!=0)
{printf("Error reading %s; this file would have been created using \"--calc-cors\"\n\n", filename);exit(1);}
...
if(strcmp(fracfile,"blank")!=0)	//either ridge, bolt, bayesr or mega with ptype 2, 3, 4, 5 or 6
{
if(mode==151)	//ridge
{
if(countcols(fracfile)!=1)
{printf("Error, %s should have one column, providing the heritability scaling (not %d)\n\n", fracfile, countcols(fracfile));exit(1);}
}
if(mode==152)	//bolt
{
if(countcols(fracfile)!=2)
{printf("Error, %s should have two columns, providing p then f2 (not %d)\n\n", fracfile, countcols(fracfile));exit(1);}
}
if(mode==153)	//bayesr
{
if(pointmass==1)
...
{printf("Error, %s should have four columns, providing fractions for the point mass, then the small, medium and large Gaussian distributions (not %d)\n\n", fracfile, countcols(fracfile));exit(1);}
}
else
{
if(countcols(fracfile)!=4)
{printf("Error, %s should have four columns, providing fractions for the tiny, small, medium and large Gaussian distributions (not %d)\n\n", fracfile, countcols(fracfile));exit(1);}
}
}
