...
//Perform GRE - modes 191, 192, 193 and 194
...
current=read_data_fly(datafile, dtype, data2, NULL, num_samples_use, keepsamps, bitstart, bitend, keeppreds_use, datainputgz, current, num_samples, num_preds, genskip, genheaders, genprobs, missingvalue, -9999, -9999, nonsnp, maxthreads);
stand_data(data2, centres+bitstart, mults+bitstart, sqdevs+bitstart, num_samples_use, bitlength, missingvalue, -1, 0, 0, NULL, 1, preds+bitstart);
...
{
//check the response
count=0;for(i=0;i<num_samples_use;i++){count+=resp[i]==-9999;}
if(count>0){printf("Error, phenotypes are missing for %d of the %d individuals; you should either restart the analysis filtering out these individuals, or use \"--dentist YES\" to pad missing values\n\n", count, num_samples_use);exit(1);}
...
{
if(value>value2+1)
{printf("Warning, to perform the analysis will require approximate %.1f Gb; you can reduce this to %.1f Gb using \"--bit-size\" (currently %d)\n\n", value, value2, bitsize);}
else
...
printf("The estimated heritability is %.4f (SD %.4f), saved in %s\n\n", value, pow(value2,.5), filename);
